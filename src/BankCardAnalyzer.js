...
const BankCardAnalyzer = () => {
  ...
  const getThemeColors = () => ({
   ...
   const ButtonStyle = ({ children, onClick, variant = 'primary', icon }) => {
    const colors = getThemeColors();
    return (
      <Button
        onClick={onClick}
        className={`
          ...
          ${variant === 'primary' ? `bg-gradient-to-r ${colors.primary}` : `bg-gradient-to-r ${colors.secondary}`}
          transform hover:scale-105 transition-all duration-200
...
All Right Reserved Capt. Esmaeili              </span>
              <span className="md:text-sm text-center">Crafted: CAPRICORN Dev</span>
            </div>
          </CardFooter>
        </Card>
      </motion.div>
